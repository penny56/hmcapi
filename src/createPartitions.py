...
def procSinglePartition(parName):
    ...
    try:
        if lock.acquire():
            ...
            (parTemp, vnicDict, vhbaDict) = createPartitionTemplate(parName, partitionDict)
            lock.release()
        if lock.acquire():
        ...
        if lock.acquire():
            if partRet != None:
                ...
                if len(vhbaDict) != 0:
                    addVhbas(partRet, parName, vhbaDict)
            else:
...
def createPartitionTemplate(parName, partitionDict):
    ...
    try:
        ...
        for propertyKey in PARTITION_API_MAP.keys():
            ...
            # for DPM R3 and above, there will use Storage Group feature for the storage setting, so here only suitable for the DPM R2 and below.
            elif propertyKey == 'vhba' and hmcVersion <= 2:
                for key in partitionDict.keys():
                    if re.match('^vhba\d-*', key):
                        # if the property name is start by vhba***
                        vhbaDict[key] = partitionDict[key]
            else:
        ...
        raise exc
    return (partitionTempl, vnicDict, vhbaDict)
...
# ------------------------------------------------------------------ #
# ----- Start of addVhbas function --------------------------------- #
# ------------------------------------------------------------------ #
def addVhbas(partUri, partName, vhbaDict):
    global hmc, cpcID
    vhbaNameSet = set()
    partID = partUri.replace('/api/partitions/','')
    # find how many vHba should we construct
    try:
        for key in vhbaDict.keys():
            if re.match('.*_name$', key):
                vhbaNameSet.add(key)
        
        for vhbaName in vhbaNameSet:
            vhbaPrefix = vhbaName[:5]
            vhbaPrefixDict = dict()
            for key in vhbaDict.keys():
                pattern = '^'+vhbaPrefix
                if re.match(pattern, key):
                    vhbaPrefixDict[key[6:]] = vhbaDict[key]
            # in vhbaPrefixDict dict, this include all the properties to create a new hba.
            # there will throw a exception while couldn't find the adapter entity by adapter name
            adapterDict = selectAdapter(hmc, vhbaPrefixDict["adapname"], cpcID)
            adapterPortUri = adapterDict[KEY_ADAPTER_URI]+'/storage-ports/0'
            hbaTempl = dict()
            hbaTempl['name'] = vhbaPrefixDict["name"]
            hbaTempl['adapter-port-uri'] = adapterPortUri
            if vhbaPrefixDict.has_key("devnum"):
                hbaTempl['device-number'] = vhbaPrefixDict["devnum"]
            if vhbaPrefixDict.has_key("desc"):
                hbaTempl['description'] = vhbaPrefixDict["desc"]
            # create the Hba
            hbaRet = createHBA(hmc, partID, hbaTempl)
            print ">>> Create vHBA for %s successfully: %s" %(partName, vhbaPrefixDict["name"])
    except HMCException as exc:
        print "[HMCEXCEPTION addVhbas]", exc.message
    except Exception as exc:
        print "[EXCEPTION addVhbas]", exc.message
# ------------------------------------------------------------------ #
# ----- End of addVhbas function ----------------------------------- #
# ------------------------------------------------------------------ #
