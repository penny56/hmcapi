...
def procSinglePartition(parName):
    ...
    try:
        if lock.acquire():
            ...
            (parTemp, vnicDict, sgSet) = createPartitionTemplate(parName, partitionDict)
            lock.release()
        if lock.acquire():
        ...
        if lock.acquire():
            if partRet != None:
                ...
                if len(sgSet) != 0:
                    constructStorageGroups(partRet, parName, sgSet)
            else:
    ...
    try:
        ...
        for propertyKey in PARTITION_API_MAP.keys():
            if partitionDict.has_key(propertyKey):
                ...
                for key in partitionDict.keys():
                    if re.match('^sg\d-*', key):
                        # add this storage groups name to the sgSet
                        sgSet.add(partitionDict[key])
            else:
        ...
        raise exc
    return (partitionTempl, vnicDict, sgSet)
...
# ------------------------------------------------------------------ #
def constructStorageGroups(partUri, parName, sgSet):
    global hmc
    partID = partUri.replace('/api/partitions/','')
    try:
        for sgName in sgSet:
