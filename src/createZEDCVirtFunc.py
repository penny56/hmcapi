...
This script intends to create Virtual Functions from zEDC adapters against specified partitions. Adding virtual function will not
depend on partition status, that it should support both dynamic and offline change to partitions. 
...
try:
    ...
    # Get adapter URI
    adap = selectAdapter(hmc, adapName, cpcID)
    adapURI = assertValue(pyObj=adap, key=KEY_ADAPTER_URI)

    # create virtual function against against input partitions one by one.
    for inputParURI in inputParURIList:
        parIndex = inputParURIList.index(inputParURI)
        inputParName = inputParNameList[parIndex]
        # Construct virtual function name for target partition
        vfName = cpcName + '-' + inputParName + '-zEDC-VF'
        # create virtual function
        vfRet = createVirtFunc(hmc,inputParURI,adapURI,vfName)
        print "\nzEDC Virtual Function <%s> was created for Partition <%s:%s>: %s"%(vfName,cpcName,inputParName,vfRet)
