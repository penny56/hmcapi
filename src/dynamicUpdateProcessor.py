...
This module is aimed to update the processor number in active state.
The new processor number should be greater than the current value, and should be enabled it be command line:

chcpu -c
chcpu -e

Example:
-hmc 9.12.35.135 -cpc M90 -parRange LNXT01,LNXT02 -procNum 6
...
def parseArgs():
    ...
    global parRange, procNum
    global logDir, logLevel, configFilename
    global argsDict
    parser = argparse.ArgumentParser(description='dynamic update the Processor parameters for partition')
...
# processor number
    ...
    if procNum != None:
        ...
        exc = HMCException("checkParams", "You should specify the new processor number")
        print exc.message
        raise exc
# log
...
# ------------------------------------------------------------------ #
# if the input parameters are acceptable and could be updated successfully, fill the parTempl and return the parTempl
...
def updatePartitionTemplate(partitionPropsTemp):
    ...
    parTempl = dict()
    if partitionPropsTemp['cp-processors'] != 0 and partitionPropsTemp['cp-processors'] < argsDict['procNum']:
        parTempl['cp-processors'] = argsDict['procNum']
    elif partitionPropsTemp['ifl-processors'] != 0 and partitionPropsTemp['ifl-processors'] < argsDict['procNum']:
        parTempl['ifl-processors'] = argsDict['procNum']
    else:
        failedDict[partitionPropsTemp['name']] = "New number of  processors should greater than the current value."
    return parTempl
# ------------------------------------------------------------------ #
...
try:
    ...
    print "******************************"
    msg = "dynamic DPM Processor Update PRSM2 partitions"
    print msg
    printParams()
    print "******************************"
    ...
    if parRange != None:
        ...
        exc = HMCException("dynamicUpdateProcessor",
    ...
    for parName in inputParNamesList:
        if parName in activeStateParNamesList:
            ...
            elif not failedDict.has_key(parName):
