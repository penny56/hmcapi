...
try:
    ...
    for sgName in sgNameListByCPC:
        print "sgName : ", sgName
       #if sgName == 'T90_RHEL21_XIV_Dedicated_SG01':
        if sgName == 'T90DBAAS6_SG' or sgName == 'T90DBAASM2_SG2':
           print "pause"
           continue
        sgURI = sgURIListByCPC[sgNameListByCPC.index(sgName)]
        sgProps = getStorageGroupProperties(hmc, sgURI=sgURI)
        # check if SG is 'fcp'-FCP or 'fc'-FICON type. 
        ...
        if sgStorType == 'fcp':
            ...
            if sgVSRsPropList != None:
                ...
                print "==== sgVSRsPropList is None"
        elif sgStorType == 'fc':
            print "== ficon sg"
            # YJ: FICON couldn't get the attached partition information here, will handle in the later part.
            pass
    ...
    for parName in allParNamesList:
        ...
        if assertValue(pyObj=parProp, key='crypto-configuration') != None:
            ...
            # remove the adapter uri array just add adapter name array
