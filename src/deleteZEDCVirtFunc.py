...
This script intends to delete specified Virtual Functions per user's input. 
...
# virtual function name or List
vfName = []

# Delete virtual function for target partition 
vfCleanAll=False
...
def parseArgs():
    ...
    parser.add_argument('-parNames','--parNames',metavar='<partition name or list>',
                        help='if a list provided script will delete all virtual functions for each partition')
    parser.add_argument('-vfName','--vfName',metavar='<virtual function name>', 
                        help='specify virtual function name or list when setting single partition')
...
def printParams():
    ...
    print("\tVirtual-Function name\t%s"%vfName)
# ------------------------------------------------------------------ #
...
try:
    ...
    print "***********************************"
    print "Delete virtual function for specified partitions"
    printParams()
    print "***********************************"
    ...
    if not vfName and len(parNames) > 1:
        vfCleanAll = True
    # Generate user-input partitions URI and Name list
    for parInfo in parResObj:
        _parName = assertValue(pyObj=parInfo,key='name')
        if _parName in parNames:
            inputParURIList.append(assertValue(pyObj=parInfo,key='object-uri'))
            inputParNameList.append(_parName)
    
    # Delete virtual functions 
    for inputParURI in inputParURIList:
        parIndex = inputParURIList.index(inputParURI)
        inputParName = inputParNameList[parIndex]
        inputParID = inputParURI.replace('/api/partitions/','')
        
        if vfCleanAll == False:
            if not vfName and len(parNames) <=1:
                print "virtual-function name should be provided"
            else:
                for _vfName in vfName:
                    ret = selectVirtFunc(hmc, inputParID, _vfName)
                    vfURI = assertValue(pyObj=ret, key=KEY_VF_URI)
                    deleteVirtFunc(hmc,virtFuncURI=vfURI)
                    print ('\tDeletion of virtual function <%s> was successful\n'%vfURI)
        else:
            # get partition properties
            parRet = getPartitionProperties(hmc,parID=inputParID)
            vfURIs = assertValue(pyObj=parRet, key='virtual-function-uris')
            if len(vfURIs) == 0:
                print "Partition <%s:%s> had no virtual function"%(cpcName,inputParName)
                
            else:
                for vfURI in vfURIs:
                    deleteVirtFunc(hmc,virtFuncURI=vfURI)
                    print '\tDeletion of virtual function <%s> was successful\n'%vfURI
                    print 'All virtual functions for partition <%s:%s> were deleted successfully\n'%(cpcName,inputParName)
